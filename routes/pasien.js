...
router.post('/', (req, res) => {
    ...
    db.query('INSERT INTO pasien (nama) VALUES (?)', [nama.trim()], (err, results) => {
        if (err) return res.status(500).send('Internal Server Error');
        const newpasien = { id: results.insertId, nama: nama.trim(), completed: false };
...
router.put('/:id', (req, res) => {
    ...
    db.query('UPDATE pasien SET nama = ?, completed = ? WHERE id = ?', [nama, completed, req.params.id], (err, results) => {
        if (err) return res.status(500).send('Internal Server Error');
